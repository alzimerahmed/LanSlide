...
**ALWAYS** use the agent system to orchestrate the 38 workflows across all projects. Main agents coordinate, sub-agents execute — this ensures the right workflow runs at the right time in the right order.

## Agent System Files
- **Main agents**: `~/.codeium/windsurf/windsurf/agents/` (9 orchestrators + README + INDEX)
- **Sub-agents**: `~/.codeium/windsurf/windsurf/sub-agents/` (38 specialists)
- **Master index**: `~/.codeium/windsurf/windsurf/agents/INDEX.md`
- **Project-local**: `.devin/agents/`, `.devin/skills/`, `.devin/rules/`, `.devin/prompt/`
- **Workflows**: removed. They were Windsurf-format docs (`auto_execution_mode` metadata Devin cannot execute). The invokable execution layer is `.devin/skills/<name>/SKILL.md` (slash commands `/name`) plus user-level ce-* skills in `%APPDATA%\devin\skills\`.
...
| Design Engineer | `agents/design-engineer.md` | UI/UX, content, CSS, animation, media, i18n, design clone, pixel analysis | 9 |
| Feature Engineer | `agents/feature-engineer.md` | Payments, uploads, search, real-time, email, PWA, state | 7 |
| Quality Engineer | `agents/quality-engineer.md` | Code review, debugging, testing, security, performance, a11y, pixel analysis | 7 |
...
| pixel-analyst | pixel-perfect-image-analysis | `39-pixel-perfect-image-analysis.md` |

## When this rule applies:
- Starting any new project (use Full Stack Orchestrator for end-to-end, or relevant main agent for targeted work)
...
- Any time a workflow from the 38-workflow system needs to be executed
- Any non-trivial task — check `docs/toolset.md` intent-map to find which sub-agents/skills/rules apply to the task type

## When this rule does NOT apply:
- Single-line edits, pure Q&A, or reading files (no orchestration needed)
- User explicitly says to skip the agent system and invoke workflows directly

## Project-type awareness
This rule originally excluded non-website projects. That exclusion was wrong — many sub-agents (code-reviewer, debugger, test-engineer, security-auditor, git-master, database-engineer, migration-specialist, performance-engineer, build-optimizer, caveman-compressor, docs-writer, pixel-analyst, vibe-coding-auditor) are universally useful across project types. For non-website projects (CLI tools, libraries, native apps like this Kotlin/Compose music app), skip web-only sub-agents (frontend-designer, css-architect, pwa-engineer, seo-specialist, etc. — see `docs/toolset.md` Excluded table) but keep the universal ones. The `docs/toolset.md` intent-map is the authoritative filter — it lists exactly which sub-agents/skills/rules apply to each task type for THIS project.
